...
/**
 * Base value for 8 bits integer blank. can be used as an unspecified blank
 */
#define DCGM_INT8_BLANK 0x70
 ...
 */
#define DCGM_INT8_IS_BLANK(val) (((val) >= DCGM_INT8_BLANK) ? 1 : 0)
...
#define DCGM_GROUP_NULL                  0x7ffffffa

/**
 * Maximum number of entities per entity group
 */
#define DCGM_GROUP_MAX_ENTITIES_V1 64
#define DCGM_GROUP_MAX_ENTITIES_V2 1024
...
typedef struct
{
    ...
    dcgmGroupEntityPair_t entityList[DCGM_GROUP_MAX_ENTITIES_V1]; //!< List of the entities that are in this group
} dcgmGroupInfo_v2;
...
typedef struct
{
    unsigned int version;                                         //!< Version Number (use dcgmGroupInfo_version3)
    unsigned int count;                                           //!< count of entityIds returned in \a entityList
    char groupName[DCGM_MAX_STR_LENGTH];                          //!< Group Name
    dcgmGroupEntityPair_t entityList[DCGM_GROUP_MAX_ENTITIES_V2]; //!< List of the entities that are in this group
} dcgmGroupInfo_v3;

/**
 * Typedef for \ref dcgmGroupInfo_v3
 */
typedef dcgmGroupInfo_v3 dcgmGroupInfo_t;

/**
 * Version 3 for \ref dcgmGroupInfo_v3
 */
#define dcgmGroupInfo_version3 MAKE_DCGM_VERSION(dcgmGroupInfo_v3, 3)

/**
 * Latest version for \ref dcgmGroupInfo_t
 */
#define dcgmGroupInfo_version dcgmGroupInfo_version3
...
/**
 * Version 2 for dcgmCpuHierarchy_t
 */
#define dcgmCpuHierarchy_version2 MAKE_DCGM_VERSION(dcgmCpuHierarchy_v2, 2)

/**
 * Latest version for \ref dcgmCpuHierarchy_t
 */
#define dcgmCpuHierarchy_version dcgmCpuHierarchy_version2
...
#define dcgmConfig_version2 MAKE_DCGM_VERSION(dcgmConfig_v2, 2)
...
/**
 * Diagnostic plugin per-entity info results v1
 *
 * Since DCGM 4.0
 */
...
/**
 * Diagnostic plugin per-entity error results v1
 *
 * Since DCGM 4.0
 */
...
typedef struct
{
    ...
    unsigned char errorIndices[DCGM_DIAG_TEST_RUN_ERROR_INDICES_MAX]; //!< Per-entity error indices for this run.
    unsigned char infoIndices[DCGM_DIAG_TEST_RUN_INFO_INDICES_MAX];   //!< Per-entity info indices for this run.
    unsigned short resultIndices[DCGM_DIAG_TEST_RUN_RESULTS_MAX];     //!< Per-entity result indices for this run.
    dcgmDiagTestAuxData_v1 auxData;
} dcgmDiagTestRun_v1;

#define DCGM_DEVICE_ID_LEN 5  //!< SKU Device ID (string)
#define DCGM_VERSION_LEN   12 //!< DCGM Version (string)

/**
 * Additional entity details for entity-centric diagnostic run.
 *
 * Since DCGM 4.0.
 */
typedef struct
...
#define DCGM_HEALTH_WATCH_MAX_INCIDENTS_V2 DCGM_GROUP_MAX_ENTITIES_V2
...
typedef struct
{
    ...
    dcgmIncidentInfo_t incidents[DCGM_HEALTH_WATCH_MAX_INCIDENTS_V2]; //!< Report of the errors detected
} dcgmHealthResponse_v5;

/**
 * Version 5 for \ref dcgmHealthResponse_v5
 */
#define dcgmHealthResponse_version5 MAKE_DCGM_VERSION(dcgmHealthResponse_v5, 5)

/**
 * Latest version for \ref dcgmHealthResponse_t
 */
#define dcgmHealthResponse_version dcgmHealthResponse_version5

/**
 * Typedef for \ref dcgmHealthResponse_v5
 */
...
typedef enum dcgmPerGpuTestIndices_enum
{
    ...
    DCGM_CONTEXT_CREATE_INDEX = 16, //!< Context create test index
    DCGM_UNKNOWN_INDEX        = 17  //!< Unknown test
...
typedef enum dcgmSoftwareTest_enum
{
    ...
    DCGM_SWTEST_FABRIC_MANAGER       = 10, //!< test for running Fabric Manager
} dcgmSoftwareTest_t;

#define DCGM_DIAG_RESPONSE_TESTS_MAX    20              //!< Maximum number of plugins that can return results.
#define DCGM_DIAG_RESPONSE_SYSTEM_ERROR DCGM_INT8_BLANK //!< Used to flag an error as a system error.

#define DCGM_DIAG_RESPONSE_ERRORS_MAX \
...
#define DCGM_DIAG_RESPONSE_RESULTS_MAX \
     ...
     * DCGM_DIAG_RESPONSE_ENTITIES_MAX)      //!< Maximum number of results that can be reported.
#define DCGM_DIAG_RESPONSE_CATEGORIES_MAX 5  //!<  One of Deployment | Hardware | Integration | Stress
...
typedef struct
{
    unsigned int version; //!< version number (dcgmDiagResponse_v11)
    ...
    dcgmDiagTestRun_v1 tests[DCGM_DIAG_RESPONSE_TESTS_MAX];          //!< Test run information for each relevant test.
    dcgmDiagEntity_v1 entities[DCGM_DIAG_RESPONSE_ENTITIES_MAX];     //!< Per-entity details.
    dcgmDiagError_v1 errors[DCGM_DIAG_RESPONSE_ERRORS_MAX];          //!< Per-entity errors.
    ...
    char _unused[DCGM_DIAG_RESPONSE_V11_UNUSED_LEN]; //!< Needed for binary compatibility with v10 and earlier.
...
#define DCGM_ENTITY_ID_LIST_LEN    2048
...
typedef struct
{
    ...
    char clocksEventMask[DCGM_CLOCKS_EVENT_MASK_LEN];  //!< Clocks event reasons to ignore as either integer mask
...
/**
 * Version 3 of dcgmProfGetMetricGroups_t. Older versions are unsupported
 */
...
typedef enum
{
    ...
    DcgmFMStatusNotStarted   = 1, //!< The Fabric Manager has not started for this GPU yet.
    DcgmFMStatusInProgress   = 2, //!< The Fabric Manager is still starting up
    DcgmFMStatusSuccess      = 3, //!< The Fabric Manager has started successfully
    DcgmFMStatusFailure      = 4, //!< The Fabric Manager finished training, but failed
    DcgmFMStatusUnrecognized = 5, //!< NVML returned an unrecognized status for the fabric manager
    DcgmFMStatusNvmlTooOld   = 6, //!< NVML returned NVML_ERROR_FUNCTION_NOT_FOUND when querying fabric manager status
